...
with open('URLs_zdrikul.txt', 'r') as f:
...
for url in urls:
    if "checkSpecjalnosc.php" in url:
        ...
        response = requests.get('http://www.plan.pwsz.legnica.edu.pl/schedule_view.php?site=show_kierunek.php&id=7')
        soup = BeautifulSoup(response.content, 'html.parser')
        name = soup.find('a', href=lambda href: href and 'specjalnosc={}'.format(specialisation) in href).text.strip().split(' (')[1]
        name = name[:-1]
        query = f"INSERT INTO field_of_study (id_faculty, fos, year, stationary, specialisation) VALUES (3, '{name}', {year}, {stationary}, '{specialisation_only}')"
        db.execute(query)
        print(query)
